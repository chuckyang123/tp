...
markdownIt.renderer.rules.fence = (tokens, idx, options, env, slf) => {
    ...
    str = lines.map((line, index) => {
        ...
        const immediateHighlight = highlightRules.flatMap(rule => rule.getHighlightType(currentLineNumber)).find(({ highlightType }) => {
            if (highlightType === HighlightRule_1.HIGHLIGHT_TYPES.WholeLine) {
                return true;
            }
            if (highlightType === HighlightRule_1.HIGHLIGHT_TYPES.WholeText) {
                return true;
            }
            return false;
        });
        ...
        if (immediateHighlight) {
            ...
            if (immediateHighlight.highlightType === HighlightRule_1.HIGHLIGHT_TYPES.WholeLine) {
                return Highlighter_1.Highlighter.highlightWholeLine(line, highlightColor);
            }
            if (immediateHighlight.highlightType === HighlightRule_1.HIGHLIGHT_TYPES.WholeText) {
                return Highlighter_1.Highlighter.highlightWholeText(line, highlightColor);
            }
        }
        // Next, handle partial text highlight logic here
        const boundsWithColors = highlightRules.flatMap(rule => rule.getHighlightType(currentLineNumber)
            .filter(({ highlightType, bounds }) => highlightType === HighlightRule_1.HIGHLIGHT_TYPES.PartialText && bounds)
            .flatMap(({ bounds, color }) => bounds.map(bound => ({ bounds: bound, color: color || '' }))));
        if (boundsWithColors.length > 0) {
            return Highlighter_1.Highlighter.highlightPartOfText(line, boundsWithColors);
        }
